...
function Books({ authReducer, booksReducer, getChapterTitles, postReview }) {
  ...
  const onSuccessPostReview = () => {
    const newReview = {
      ...
      user: {
        profile_img: {
          name: authReducer?.userData?.profile_img?.name,
        },
        firstName: authReducer?.userData?.firstName,
        lastName: authReducer?.userData?.lastName,
      },
    };

    let copyReviews = [...allReviews];
    copyReviews?.shift(newReview);
                  ...
                  return (
                    <div className={`row ${CHECK % 2 === 0 && "bg-color"} `}>
                      ...
                      <div className="col-lg-11 col-md-10 col-10">
                        <h1 className="volume-head">{ele?.title}</h1>
